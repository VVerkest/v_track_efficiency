void P6GeantPlot(){
  ...
  TFile* p6GeantFILE = new TFile( (inFileName).c_str() , "READ" );     // Read in data file

  TFile *fout = new TFile( (fileSaveName).c_str() ,"RECREATE");    // create a new Root file

  //  gather histograms
  TH3D* h_all_EtaPhiPt = (TH3D*) p6GeantFILE->Get("all_EtaPhiPt");
  TH3D* h_allcons_EtaPhiPt = (TH3D*) p6GeantFILE->Get("allcons_EtaPhiPt");
  TH3D* h_lead_EtaPhiPt = (TH3D*) p6GeantFILE->Get("lead_EtaPhiPt");
  TH3D* h_leadcons_EtaPhiPt = (TH3D*) p6GeantFILE->Get("leadcons_EtaPhiPt"); 
