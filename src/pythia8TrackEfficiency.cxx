...
int main() {
  ...
  for (unsigned iEvent = 0; iEvent < 100000; ++iEvent) {    //  BEGIN EVENT LOOP
	...
	//  All particles
	if ( chargeBias == "charged" &&  !pythia.event[h].isCharged() )  { continue; }  // removes neutral particles
	if ( ptCutStatus == "ptCut" && pythia.event[h].pT() < 0.2)                { continue; }  // removes particles below 0.2 GeV
	if ( etaCutStatus == "etaCut" && abs(pythia.event[h].eta()) > 1.0 )   { continue; }  // removes particles with eta>|1|
	...
	if ( useEfficiency == false ) { all_rawJets.push_back(current); }
	  ...
	  if ( chargeBias == "all" && !pythia.event[h].isCharged() ) { all_rawJets.push_back(current); }  // keep all neutral particles
	  if ( effic_num < 0.8 && pythia.event[h].isCharged() ) { all_rawJets.push_back(current); }  // keep 80% of charged particles
	}
      }
    }    //  end particle loop
